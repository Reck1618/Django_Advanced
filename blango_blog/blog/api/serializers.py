...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Comment
        fields = ["id", "creator", "content", "created_at", "modified_at"]
        read_only_fields = ["created_at", "modified_at"]

class PostSerializer(serializers.ModelSerializer):
    tags = serializers.SlugRelatedField(slug_field="value", many=True, queryset=Tag.objects.all())
    author = serializers.HyperlinkedRelatedField(queryset=User.objects.all(), view_name="api_user_details", lookup_field="email")
    class Meta:
        model = Post
        fields = "__all__"
